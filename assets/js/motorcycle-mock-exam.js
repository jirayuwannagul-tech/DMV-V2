/**
 * motorcycle-mock-exam.js — สอบจำลองมอเตอร์ไซค์แบบจับเวลา (motorcycle-mock-exam.html)
 * โครงสร้างเหมือนกับ mock-exam.js ทุกประการ เปลี่ยนแหล่งข้อมูลเป็น MOTO_TESTS
 *
 * จำนวนคำถามและเกณฑ์ผ่านคำนวณแบบไดนามิกจากขนาดคลังข้อสอบ MOTO_TESTS
 * (สูงสุด 46 ข้อ เกณฑ์ผ่าน ~83% เหมือนรถยนต์) เพื่อให้ใช้งานได้ทันที
 * ไม่ว่าจะมีคำถามอยู่กี่ข้อก็ตาม — เมื่อเพิ่มคำถามจริงครบแล้วค่าจะปรับเองอัตโนมัติ
 */

/* ── Constants ──────────────────────────────────────────────── */
const POOL_SIZE       = MOTO_TESTS.length;
const TOTAL_QUESTIONS = Math.max(1, Math.min(46, POOL_SIZE));
const PASS_THRESHOLD  = Math.max(1, Math.ceil(TOTAL_QUESTIONS * 0.83));
...
function initStartScreen() {
  document.getElementById('mock-start-subtitle').innerHTML =
    `จำลองการสอบใบขับขี่มอเตอร์ไซค์จริง สุ่มคำถาม <strong>${TOTAL_QUESTIONS} ข้อ</strong> จากคลัง ${POOL_SIZE} ข้อ<br>
     ต้องตอบถูกอย่างน้อย <strong>${PASS_THRESHOLD} ข้อ</strong> จึงจะผ่าน`;
  document.getElementById('rule-total-icon').textContent = TOTAL_QUESTIONS;
  document.getElementById('rule-total-text').innerHTML =
    `คำถามทั้งหมด <strong>${TOTAL_QUESTIONS} ข้อ</strong> — สุ่มจากคลัง ${POOL_SIZE} ข้อ ทุกครั้งจะได้คำถามต่างกัน`;
  const passPct = Math.round((PASS_THRESHOLD / TOTAL_QUESTIONS) * 100);
  document.getElementById('rule-pass-text').innerHTML =
    `เกณฑ์ผ่าน <strong>${PASS_THRESHOLD} ข้อขึ้นไป (≥${passPct}%)</strong> — เหมือนข้อสอบจริงของ California DMV`;
}

function pickQuestions() {
  const shuffled = shuffleArray(MOTO_TESTS);
...
function startExam() {
  ...
  document.getElementById('exam-title').textContent   = `California DMV — ${TOTAL_QUESTIONS} ข้อ`;
  initUI();
  renderQuestion(0);
  startTimer();
}
...
function showSummary() {
  ...
  document.getElementById('result-note').textContent  =
    `คะแนน ${score}/${total} — ${pct}% — เกณฑ์ผ่าน ${PASS_THRESHOLD} ข้อ`;
  ...
  document.getElementById('summary-pass-threshold').textContent = PASS_THRESHOLD;
  document.getElementById('summary-pass-total').textContent  = total;
...
/* ── Traffic sign SVG (mirrors motorcycle.js) ───────────────── */
