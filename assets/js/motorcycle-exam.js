/**
 * motorcycle-exam.js — Logic สำหรับหน้าทำข้อสอบมอเตอร์ไซค์เป็นชุด (motorcycle-exam.html)
 * โครงสร้างเหมือนกับ exam.js ทุกประการ เปลี่ยนแหล่งข้อมูลเป็น MOTO_TESTS
 */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const availableTests = [...new Set(MOTO_TESTS.map(q => q.test))];
  const maxTest = availableTests.length ? Math.max(...availableTests) : 1;

  if (testNumber < 1 || testNumber > maxTest) {
    window.location.href = 'index.html';
    return;
  }

  questions = getMotoTestQuestions(testNumber);
...
function initUI() {
  // header labels
  document.getElementById('exam-label').textContent = 'ชุดข้อสอบมอเตอร์ไซค์';
  document.getElementById('exam-title').textContent = `ชุดที่ ${testNumber}`;
  document.title = `DMV — มอเตอร์ไซค์ ชุดที่ ${testNumber}`;
